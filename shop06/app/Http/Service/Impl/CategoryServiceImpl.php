...
class CategoryServiceImpl implements ICategoryService
{
     ...
     * @return array
     */
    public static function getTree(Object $Object,$parent_id=0,$field_id,$field_pid,$lv=0,$ids=[],$arr=[]){
        if (empty($ids)) foreach ($Object as $o){ $ids[] = $o->$field_pid; }
        foreach ($Object as $k=>$vo){
            if ($vo->$field_pid==$parent_id){
                for ($i = 0;$i < $lv;$i++){ $vo->pre .= '|--'; }
                $arr[] = $vo;
                if (in_array($vo->$field_id,$ids)) $arr = self::getTree($Object,$vo->$field_id,'id','parent_id',$lv+1,$ids,$arr);
            }
        }
        return $arr;
    }
